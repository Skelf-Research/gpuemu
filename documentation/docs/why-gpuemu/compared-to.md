...
|---|---|---|
| **`torch.testing.assert_close`** | Standard, simple, in-tree, runs anywhere | One shape, one dtype, one seed; no op-schema fuzz; no fp64 reference; no per-op calibrated tolerances. **P1** measured this catches 0/9 LLM-style buggy kernels in the gpuemu corpus. |
